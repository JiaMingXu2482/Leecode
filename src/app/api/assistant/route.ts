  ...
  const messages: ChatMessage[] = [
    {
      ...
      content: [
        ...
        `配额铁律: 用户的计划是每天 ${settings.newPerDay} 道新题（复习题不算在内，另计）。这是硬约束，任何操作后每天“当天新题”都应尽量等于 ${settings.newPerDay} 道。例外——顺延：昨天没做完的新题会被系统自动顺延到今天，叠加在当天配额之上（快照里标注“顺延自X”），比如当天4道+顺延2道=6道属于正常，不要把顺延题搬走或因此改配额。移动/新增题目时绝不能把某天的“当天新题”堆到超过 ${settings.newPerDay} 道：如果会超额，就把那天原本排的、尚未完成的新题按超出的数量顺延到后面的日子（级联），使每天仍是 ${settings.newPerDay} 道；已完成的新题不动。快照里标了“当天新题超出/少于目标”的日子要主动提醒用户。只有用户明确表达“以后每天做 X 道”这种长期意愿时才调用 set_new_per_day 改配额；用户只是要求某一天临时多做/少做几道，属于一次性例外：照做即可，但绝不要因此改配额。`,
